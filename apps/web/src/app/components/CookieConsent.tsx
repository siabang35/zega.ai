...
export function CookieConsent({ onNavigatePrivacy, onNavigateTerms }: { onNavigatePrivacy?: () => void; onNavigateTerms?: () => void }) {
  ...
  return createPortal(
    ...
    >
      <div className="group relative overflow-hidden rounded-2xl border border-slate-200/80 dark:border-slate-800/80 p-5 md:p-6 font-sans shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.7)] transition-all duration-300">
        {/* Animated Border Beam Cycle (Replaces static top gradient line) */}
        <div
          className="pointer-events-none absolute -inset-[150%] rounded-full opacity-60 group-hover:opacity-90 transition-opacity duration-500 z-0"
          style={{
            background: `conic-gradient(from 0deg, transparent 0deg, #14F195 35deg, #9945FF 80deg, #00FFA3 120deg, transparent 160deg)`,
            animation: `cookie-spin-beam 6s linear infinite`,
          }}
        />
        <div className="pointer-events-none absolute inset-[1px] rounded-[15px] bg-white/95 dark:bg-slate-950/95 backdrop-blur-2xl z-0" />
        ...
        <div className="relative z-10 space-y-3.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 transition-colors cursor-pointer"
          >
