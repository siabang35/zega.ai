// Multi-User Test Script targeting Production Render Backend (https://zega-ai.onrender.com)

async function testRenderBackend() {
  const targetApi = 'https://zega-ai.onrender.com';
  console.log(`🚀 Starting Production Render Multi-User Tests...\n🌐 Active API Target: ${targetApi}\n`);
  ...
  console.log('════════════════════════════════════════════════════════');
  console.log('  PHASE 1: INVOICE GENERATION & TELEGRAM DISPATCH (RENDER)');
  console.log('════════════════════════════════════════════════════════\n');
  ...
  const fakeSigs = [
    `5J4X2K3k8y9z7W6v5u4t3s2r1qxP9N8n7m6k5K4J3H2G1FxE9D8C7B6A543219876543219876543219876${timestamp}A`,
    `4K3J2h1g9F9E8d7C6B5A4321xP9n8N7m6k5K4J3h2G1FxE9d8C7B6A543219876543219876543219876${timestamp}B`,
    `3h2G1F9E8d7C6B5A4321xP9n8N7m6k5K4J3h2G1FxE9d8C7B6A543219876543219876543219876${timestamp}C`,
  ];

  console.log('════════════════════════════════════════════════════════');
  console.log('  PHASE 2: SETTLEMENT & MULTI-STATUS RECEIPT DISPATCH (RENDER)');
  console.log('════════════════════════════════════════════════════════\n');
  ...
  for (const st of settlementTests) {
    console.log(`🧪 ${st.name}...`);
    if (!st.refKey) {
      console.log(`  ⏭️ Skipped — no reference key\n`);
      continue;
    }
    try {
      const res = await fetch(`${targetApi}/v1/zeroclaw/settlement/record`, {
        ...
        body: JSON.stringify({
          ...
          memo: `Render Test Settlement for ${st.target}`,
          isDemo: true
        })
      });
  ...
  console.log('════════════════════════════════════════════════════════');
  console.log('  RENDER BACKEND TEST COMPLETE');
  console.log('════════════════════════════════════════════════════════');
}
