...
-- ============================================================
-- ZEGA AI PLATFORM
-- BACKEND CANONICAL APPLICATION USER RPC HARDENING
-- Migration: 20260822010000_fix_backend_canonical_user_product_rpc.sql
-- ============================================================

-- 1. HARDENED STORE PRODUCT CREATION RPC (AUTHENTICATED REAL USERS & SERVICE_ROLE)
...
BEGIN
    ...
    -- Verify application user is active
    IF NOT EXISTS (
        SELECT 1 FROM public.users
        WHERE id = v_app_user_id AND COALESCE(status, 'active') <> 'disabled'
    ) THEN
        RAISE EXCEPTION 'USER_NOT_AUTHORIZED: Application user profile is inactive or disabled'
            USING ERRCODE = '42501';
    END IF;
