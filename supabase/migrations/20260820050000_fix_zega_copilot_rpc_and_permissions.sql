-- ============================================================================
-- ZEGA AI PLATFORM — FIX ZEGA COPILOT RPC TENANT COLUMNS & EXECUTE PERMISSIONS
-- Migration: 20260820050000_fix_zega_copilot_rpc_and_permissions.sql
-- ============================================================================

BEGIN;

-- 1. ENSURE organization_id AND workspace_id COLUMNS EXIST ON umkm_zega_copilot_messages
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'umkm_zega_copilot_messages' AND column_name = 'organization_id'
    ) THEN
        ALTER TABLE public.umkm_zega_copilot_messages ADD COLUMN organization_id UUID;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'umkm_zega_copilot_messages' AND column_name = 'workspace_id'
    ) THEN
        ALTER TABLE public.umkm_zega_copilot_messages ADD COLUMN workspace_id UUID;
    END IF;
END $$;

-- 2. GRANT EXECUTE ON HELPER & IDENTITY FUNCTIONS TO ALL ROLES (ALLOWS PostgREST CALLS)
GRANT EXECUTE ON FUNCTION public.fn_can_access_umkm_store(UUID, UUID, UUID) TO authenticated, anon, service_role, public;
GRANT EXECUTE ON FUNCTION public.fn_is_org_member(UUID) TO authenticated, anon, service_role, public;
GRANT EXECUTE ON FUNCTION public.fn_current_app_user_id() TO authenticated, anon, service_role, public;
GRANT EXECUTE ON FUNCTION public.fn_get_or_create_current_app_user() TO authenticated, anon, service_role, public;

-- 3. RE-CREATE fn_save_zega_copilot_message TO POPULATE organization_id AND workspace_id
...
BEGIN
    ...
    v_final_org_id := COALESCE(v_store_org_id, v_chat.organization_id);
    v_final_ws_id := COALESCE(v_store_ws_id, v_chat.workspace_id);

    -- E. Strict Multi-Tenant Copilot Chat Ownership Validation (SECURITY DEFINER ENFORCED)
    IF NOT (
        ...
        OR public.fn_can_access_umkm_store(v_chat.store_id, v_final_org_id, v_final_ws_id)
        OR (v_final_org_id IS NOT NULL AND public.fn_is_org_member(v_final_org_id))
    ) THEN
        RETURN jsonb_build_object(
            'ok', FALSE,
            'errorCode', '42501',
    ...
    -- G. Insert Message Row Atomically with tenant columns
...
-- 4. GRANT EXECUTE TO ALL ROLES (ALLOWS PostgREST CALLS UNDER EXTERNAL AUTH / ANON SESSIONS)
GRANT EXECUTE ON FUNCTION public.fn_save_zega_copilot_message(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, INT, INT, TEXT) TO authenticated, anon, service_role, public;

-- 5. HARDEN RLS POLICIES FOR BOTH AUTHENTICATED AND ANON (SECURITY CHECKED VIA CHAT/STORE ACCESS)
