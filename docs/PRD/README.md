...
| 13 | [Enterprise Security & Supabase Hardening](./13-ENTERPRISE-SECURITY-CDN-SUPABASE-HARDENING.md) | `13-ENTERPRISE-SECURITY-CDN-SUPABASE-HARDENING.md` | Cloudflare Turnstile bot defense, R2 CDN asset architecture, Brevo email OTP, Supabase master migrations |
| 14 | [Authentication, Sessions & Master Schema](./14-AUTHENTICATION-SESSIONS-SUPABASE-MASTER-SCHEMA.md) | `14-AUTHENTICATION-SESSIONS-SUPABASE-MASTER-SCHEMA.md` | Console access guards, session cookies, local telemetry caching, master Supabase migration suite |
| 15 | [Newsletter & Routing Specification](./15-NEWSLETTER-ROUTING-AND-COMPLIANCE-SPEC.md) | `15-NEWSLETTER-ROUTING-AND-COMPLIANCE-SPEC.md` | GDPR-compliant opt-in subscriptions table, HTML5 History API URL routing, SPA fallback rewrite rules |
...
| 22 | [ZeroClaw Solana Multi-LLM Agent Runtime](./22-ZEROCLAW-SOLANA-MULTI-LLM-AGENT-SPEC.md) | `22-ZEROCLAW-SOLANA-MULTI-LLM-AGENT-SPEC.md` | Multi-LLM tiered failover (Groq, Gemini, OpenRouter, Jatevo, 9Router, HF), Keyless Solana Pay QR, Devnet RPC reconciliation |
| 23 | [ZeroClaw Terminal Operator Guide (EN)](./23-ZEROCLAW-TERMINAL-USER-GUIDE.md) | `23-ZEROCLAW-TERMINAL-USER-GUIDE.md` | Step-by-step operational workflows for UMKM retail merchants and Enterprise governance operators |
| 24 | [ZeroClaw Terminal Panduan Pengguna (ID)](./24-ZEROCLAW-TERMINAL-PANDUAN-PENGGUNA-ID.md) | `24-ZEROCLAW-TERMINAL-PANDUAN-PENGGUNA-ID.md` | Panduan operasional Bahasa Indonesia: POS Kasir, QRIS Solana Pay, ATA USDC resolution, RLS Supabase private feed |
| 25 | [ZeroClaw Terminal Redesign & Finance Reconciliation](./25-ZEROCLAW-TERMINAL-REDESIGN-AND-REAL-FINANCE-RECONCILIATION-SPEC.md) | `25-ZEROCLAW-TERMINAL-REDESIGN-AND-REAL-FINANCE-RECONCILIATION-SPEC.md` | Merged overview/generator UI, 44-char Base58 Solana Pay ref keys, direct RPC signature verification, dynamic Finance Overview |
| 26 | [Production Reconciliation & OWASP Security](./26-PRODUCTION-RECONCILIATION-AND-OWASP-SECURITY-SPEC.md) | `26-PRODUCTION-RECONCILIATION-AND-OWASP-SECURITY-SPEC.md` | Distributed deployment topology (Vercel/Render), 4-tier client RPC resolution engine, 5-layer backend validation pipeline |
...
| 28 | [Strict Privy Auth & Solana Pay Checkout](./28-STRICT-PRIVY-AUTH-DEMO-PURGE-AND-SOLANA-PAY-SPEC.md) | `28-STRICT-PRIVY-AUTH-DEMO-PURGE-AND-SOLANA-PAY-SPEC.md` | Demo mode purge, 100% authenticated environment, standalone checkout route (`/checkout/:id`), multi-tier RPC resolution |
...
| 30 | [Dynamic Multi-User Solana Payment Reconciliation](./30-DYNAMIC-MULTI-USER-SOLANA-PAYMENT-RECONCILIATION-SPEC.md) | `30-DYNAMIC-MULTI-USER-SOLANA-PAYMENT-RECONCILIATION-SPEC.md` | Dynamic keyless wallet derivation, zero-RPC USDC ATA formula, 4-stage multi-user settlement reconciliation pipeline |
...
| 42 | [ZeroClaw Secure Withdrawal Vault & QR Barcode Audit Spec](./42-ZEROCLAW-SECURE-WITHDRAWAL-VAULT-AND-QR-BARCODE-AUDIT-SPEC.md) | `42-ZEROCLAW-SECURE-WITHDRAWAL-VAULT-AND-QR-BARCODE-AUDIT-SPEC.md` | OWASP V3 multi-layer secure withdrawal vault, Brevo 2-step email OTP authorization, live Solana Devnet RPC address scanner, embedded QR code / barcode camera viewport scanner, anti-replay SHA-256 signatures, DB anti-exploit guards, and Cloudflare R2 audit proof receipts |
...
| 47 | [AI Inventory Swarm Security & Orchestration Spec](./47-AI-INVENTORY-SWARM-PRODUCTION-SECURITY-AND-ORCHESTRATION-SPEC.md) | `47-AI-INVENTORY-SWARM-PRODUCTION-SECURITY-AND-ORCHESTRATION-SPEC.md` | Production-grade AI Inventory Auto-Stock Swarm, multi-tenant RLS isolation (`organization_members` check), non-blocking audit logging, backend REST authorization, tool-level authority enforcement, and interactive frontend dashboard |
| 48 | [Universal AI Chat Gateway & Multi-Tenant Hardening Spec](./48-UNIVERSAL-AI-CHAT-GATEWAY-AND-MULTI-TENANT-HARDENING-SPEC.md) | `48-UNIVERSAL-AI-CHAT-GATEWAY-AND-MULTI-TENANT-HARDENING-SPEC.md` | Production-grade Universal AI Chat Gateway, multi-swarm orchestration engine (6 domains), strict server-side JWT tenant resolution, empirical zero-data grounding rules, CORS header hardening (`x-zega-ai-language`), empty body DELETE handling, and mutation approval tokens |
